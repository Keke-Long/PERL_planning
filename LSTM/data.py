...
def create_dateset(info, look_back=10):
    ...
    for i in range(len(info) - 2 * look_back - 1):
        a = info[i: (i+look_back), :] # train info
        dataX.append(a)
        b = info[(i+look_back):(i + 2 * look_back), 5] # test info
        dataY.append(b)
...
def load_data():
    ...
    info = np.hstack([seq, pre_x, pre_y, sub_x, sub_y, sub_a])
    ...
    return train_X, train_Y, test_X, test_Y, a_min, a_max
