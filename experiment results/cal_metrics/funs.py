...
def analyze_ttc(file_path):
    ...
    for baseline, (pos_col, vel_col) in baselines.items():
        if pos_col in data.columns and vel_col in data.columns:
            min_ttc, ttc_under_2, _ = calculate_ttc(
                data,
                pos_col, vel_col,
                'Y4', 'v4'
            )
...
def calculate_total_fuel_per_100km(data, v_col, a_col):
    ...
    for index, row in data.iterrows():
        ...
        if pd.notna(v) and pd.notna(a):
            ...
            if index > 0:
                total_distance += (v + previous_v) / 2 * 0.1
            previous_v = v
...
def analyze_fuel_consumption(data):
    trajectories = {
        'Baseline 1': ('v5', 'a5'),
        'Baseline 2': ('v5_mpc_base2', 'a5_mpc_base2'),
        'Baseline 3': ('v5_mpc_base3', 'a5_mpc_base3')
    }
    # Calculate fuel consumption for each trajectory
    for label, (v_col, a_col) in trajectories.items():
        if v_col in data.columns and a_col in data.columns:
            # Truncate data to match the shortest sequence length
            valid_length = data[v_col].dropna().shape[0]
            data[v_col] = data[v_col].iloc[:valid_length]
            fuel_per_100km = calculate_total_fuel_per_100km(data, v_col, a_col)
...
def calculate_relative_dampening_ratio(data, local_accel_column, reference_accel_column):
    ...
    if local_accel_column in data.columns and reference_accel_column in data.columns:
